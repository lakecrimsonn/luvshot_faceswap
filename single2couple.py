...
def single2couple(background_img_path, source_img_path_1, source_img_path_2):
    COMBINATED_DIR = 'combinated_image'
    os.makedirs(COMBINATED_DIR, exist_ok=True)
    landmarkModel = LandmarkModel(name='landmarks')
    landmarkModel.prepare(ctx_id= 0, det_thresh=0.6, det_size=(640,640))

    source_aligned_img_1 =  faces_align(landmarkModel, source_img_path_1)
    source_aligned_img_2 =  faces_align(landmarkModel, source_img_path_2)
    background_aligned_img = faces_align(landmarkModel, background_img_path)

    # temp 
    male = source_aligned_img_1
    female = source_aligned_img_2

    # src[0] : image
    # src[1] : back_matrix
    # src[2] : gender, 0 : female, 1 : male

    # check gender
    for src in source_aligned_img_1:
        if (src[2] == 0):
            female = source_aligned_img_1
            print('source 1 is a female')
        else:
            male = source_aligned_img_1
            print('source 1 is a male')
    
    for src in source_aligned_img_2:
        if(src[2] == 0):
            female = source_aligned_img_2
            print('source 2 is a female')

        else:
            male = source_aligned_img_2
            print('source 2 is a male')

    # cv2.imshow('male', male[0][0])
    # cv2.waitKey(0)

    # cv2.imshow('female', female[0][0])
    # cv2.waitKey(0)
    ...
    for idx, background in enumerate(background_aligned_img):
        ...
        if(background[2] == 0):
            id_emb, id_feature = get_id_emb_from_image(id_net, female[0][0])
            ...
            id_emb, id_feature = get_id_emb_from_image(id_net, male[0][0])
...
def gfpgan_gogo(img):
    ...
    print('single2couple completed !')


if __name__ == '__main__':
    # background image, image1, image2
    single2couple('data/test_couple.jpg', 'data/zzan.png', 'data/karina.jpg')